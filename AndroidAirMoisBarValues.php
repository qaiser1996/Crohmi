...
include 'assets/php/conn.php';
	...
	$hours = [];
	$sql = "SELECT date, AVG(airMoisture) as avg FROM `reading` WHERE (date >(CURDATE()))";
	...
	for ($i=0; $i<30;$i++){
		$j=$i+1;
		$sql = "SELECT date, AVG(airMoisture) as avg FROM `reading` WHERE (date>(CURDATE()-INTERVAL $j DAY)) AND (date<(CURDATE()- INTERVAL $i DAY))";
		$result = mysqli_query($conn,$sql);
		if (mysqli_num_rows($result)>0){
			$row = mysqli_fetch_assoc($result);
			if ($row['date']){
